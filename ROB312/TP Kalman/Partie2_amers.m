...
rng(123456) % imposer la graine de génération de nombres pseudo-aléatoire pour la répétabilit?
...
%Qf = diag([0.05,0.05,0.0005]);
%Rfi = diag([20,1*pi/180].^2); % matrice de covariance du bruit de mesure pour chaque amer
...
for tk = 2:(T/dt)
    ...
    % prediction (? compléter: variables X_hat et P_hat)
    F = [1, 0, -dt*V*sin(X_hat(3));0, 1, dt*V*cos(X_hat(3));0, 0, 1;]; % matrice de dynamique
    X_hat = [X_hat(1) + V*dt*cos(X_hat(3)); X_hat(2) + V*dt*sin(X_hat(3)); X_hat(3) + dt*omega]; % état estim? prédit
    ...
    % validit? de la mesure réelle (gestion de la fréquence)
    %dt_mesure = 0.1; % (s)
    dt_mesure = 0.1;
    ...
    % correction (? compléter: variables K, P_hat, inno, X_hat)
    H = [];
    if is_measurementValid
        ...
        for i = 1:size(amers,2)
            ...
            %Y_hati = [norm(Delta_hat),atan(Delta_hat(2)/Delta_hat(1))-X_hat(3)]';
    ...
    grid on
    axis equal
    legend('Position vraie', 'position estime','Location','best')
    drawnow
end
